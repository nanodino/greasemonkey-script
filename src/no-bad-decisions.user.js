...
// // @author NANCY CANTU // @match https://github.com/*
// // @grant none
...
(function () {
  ...
  function askForConfirmation() {
    return confirm(
      "Are you going to stick around to monitor these changes? It is after 3pm"
    );
  }
  function mergePullRequest(event) {
    if (
      confirm(
        "Are you sure you don't want to SQUASH and merge here? And is it not too late to be merging?"
      )
    ) {
  ...
  function main() {
    const mergeButton = document.querySelector(
      "#partial-pull-merging > div.merge-pr.js-merge-pr.js-details-container.Details.is-merging.is-updating-via-merge > div > div > div > div > div.merge-message > div > div > button.merge-box-button.btn-group-merge.rounded-left-2.btn.btn-primary.BtnGroup-item.js-details-target.hx_create-pr-button"
    );
